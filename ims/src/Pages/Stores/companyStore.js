...
export default function Companystore() {
  const [companystore, Setcompanystore] = React.useState([]);
  ...
  // delete api call
  const deletecompanystore = async (id) => {
    let result = await fetch(`http://localhost:5000/companystore/${id}`, {
      method: "Delete",
    });

    if (result) {
      alert("Your companystore list is deleted:");
      result = await result.json();
      getcompanystore();
    }
  };
    ...
    <div className="">
      <TableContainer component={Paper}>
        <div style={styles.container}>
          <h1 style={styles.heading}>CompanyStore List:</h1>
          <NavLink to="/add-company-stores">
            <AddIcon
              style={{
            ...
            {companystore.map((data, index) => (
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => deletecompanystore(data._id)}
