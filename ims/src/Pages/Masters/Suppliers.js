...
export default function Suppliers() {
  ...
  const deleteContractor = async (id) => {
    let result = await fetch(`http://localhost:5000/suppliers/${id}`, {
      method: "Delete",
    });

    if (result) {
      alert("Your Suppliers list is deleted:");
      result = await result.json();
      getSuppliers();
    }
  };
    ...
    <div className="">
      <TableContainer component={Paper}>
        <div style={styles.container}>
          <h1 style={styles.heading}>Suppliers List:</h1>
          <NavLink to="/add-suppliers">
            <AddIcon
              style={{
            ...
            {suppliers.map((data, index) => (
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => deleteContractor(data._id)}
