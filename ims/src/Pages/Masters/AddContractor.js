...
export default function AddContractor() {
  ...
  const addProduct = async () => {
    ...
    console.log(userId);
    let result = await fetch("http://localhost:5000/add-contractor", {
      method: "post",
      body: JSON.stringify({
        name,
        code,
  ...
  return (
    <div className="add-product-container">
      <Box
        ...
        autoComplete="off"
      >
        <h1 className="add-product-heading"> Add-Contractor :</h1>
        ...
        >
          {name && code && email && mobileno && gstno ? (
            <NavLink to="/contractors" className="link">
              Submit
            </NavLink>
          ) : (
            <NavLink to="/add-contractor" className="link">
              Submit
            </NavLink>
          )}
        </Button>
